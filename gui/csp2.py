...
class PuzzleUI:
    def __init__(self, master):
        ...
        self.performance_data = {
            "Random Generation": {"time": 0, "nodes_expanded": 0, "depth": 0, "path_length": 0},
            "Backtracking": {"time": 0, "nodes_expanded": 0, "depth": 0, "path_length": 0},
            "Backtracking + AC-3": {"time": 0, "nodes_expanded": 0, "depth": 0, "path_length": 0}
        }
    ...
    def init_buttons(self):
        ...
        self.btn_compare = tk.Button(self.master, text="Compare Performance", width=20, 
                                   command=self.compare_performance, font=self.custom_font, 
                                   bg="#FF5722", fg="white")
    ...
    def auto_random_step(self):
        ...
        if self.try_count >= self.max_tries:
            ...
            self.performance_data["Random Generation"]["time"] = time.perf_counter() - self.start_time
            self.performance_data["Random Generation"]["nodes_expanded"] = self.try_count
            self.performance_data["Random Generation"]["depth"] = len(self.history)
            self.performance_data["Random Generation"]["path_length"] = len(self.history)
    ...
    def solve_with_backtracking(self):
        ...
        def backtracking(pos=0, board=None):
            if board is None:
            ...
            if pos == 0:
                nums = random.sample(range(9), 9)
            else:
                used = {board[i][j] for i in range(3) for j in range(3) if board[i][j] is not None}
                nums = [n for n in range(9) if n not in used]
                random.shuffle(nums)

            for num in nums:
                self.solver.nodes_expanded += 1
                board[row][col] = num
                if check_constraints(board):
                    self.history.append([row[:] for row in board])
        ...
        self.solver.reset_metrics()
        start_time = time.perf_counter()
        self.history = []  # Xóa lịch sử trước khi chạy
        self.current_step = -1
        
        success = backtracking()
        ...
        self.performance_data["Backtracking"]["time"] = time.perf_counter() - start_time
        self.performance_data["Backtracking"]["nodes_expanded"] = self.solver.nodes_expanded
        self.performance_data["Backtracking"]["depth"] = len(self.history)
        self.performance_data["Backtracking"]["path_length"] = len(self.history)

        self.status_label.config(text="Đang hiển thị quá trình tìm bằng Backtracking...")
        self.try_label.config(text=f"Số bước thử: {len(self.history)}")
        self.current_step = 0
    ...
    def solve_with_bt_ac3(self):
        ...
        self.performance_data["Backtracking + AC-3"]["time"] = time.perf_counter() - start_time
        self.performance_data["Backtracking + AC-3"]["nodes_expanded"] = self.solver.nodes_expanded
        self.performance_data["Backtracking + AC-3"]["depth"] = len(self.btac3_steps)
        self.performance_data["Backtracking + AC-3"]["path_length"] = len(self.btac3_steps)
    ...
    def compare_performance(self):
        try:
            ...
            messagebox.showerror("Lỗi", "Vui lòng cài đặt matplotlib để xem biểu đồ so sánh!\nDùng lệnh: pip install matplotlib")
            return

        algorithms = list(self.performance_data.keys())
        times = [self.performance_data[algo]["time"] for algo in algorithms]
        nodes_expanded = [self.performance_data[algo]["nodes_expanded"] for algo in algorithms]
        depths = [self.performance_data[algo]["depth"] for algo in algorithms]
        path_lengths = [self.performance_data[algo]["path_length"] for algo in algorithms]

        fig, axs = plt.subplots(2, 2, figsize=(12, 10))
        fig.suptitle('So sánh hiệu suất các thuật toán', fontsize=16)

        axs[0, 0].bar(algorithms, times, color=['blue', 'blue', 'blue'])
        axs[0, 0].set_title('Thời gian (s)')
        axs[0, 0].set_ylabel('Giây')

        axs[0, 1].bar(algorithms, nodes_expanded, color=['red', 'red', 'red'])
        axs[0, 1].set_title('Số node mở rộng')
        axs[0, 1].set_ylabel('Số node')

        axs[1, 0].bar(algorithms, depths, color=['green', 'green', 'green'])
        axs[1, 0].set_title('Độ sâu lời giải')
        axs[1, 0].set_ylabel('Số bước')

        axs[1, 1].bar(algorithms, path_lengths, color=['orange', 'orange', 'orange'])
        axs[1, 1].set_title('Độ dài lời giải')
